...
%For action recognition, in order to effectively adapt intensity-based methods for depth data, we need satisfy two major factors. Firstly, a robust feature representation is extremely important to exactly capture motion information. Secondly, to ensure that a motion contains full information in depth video, merging depth information into feature representation is an indispensable requirement. However, the recent proposed methods do not combine two the factors completely. Works \cite{yang2012recognizing, xia2013spatio} consider depth value as intensity value and adapt the intensity-based techniques. Although, they can achieve reasonable results, they deal with many limitations. The basis idea in \cite{yang2012recognizing} is to accumulate global activities from sequences of segmented human postures in three virtual views. The postures can be obtained by projecting depth maps without background onto three orthogonal planes (i.e. correspond to three view positions: front, side and top). With this approach, feature representations may be ensured including depth information. Due to the density of human postures in performing movements, accumulating global motion information can make confusions in the feature representations of different actions. \cite{xia2013spatio} adopts intensity-based approach proposed in \cite{dollar2005behavior}. In this approach, authors present a filtering method to extract Spatio-Temporal Interest Points of depth video (DSTIP). For challenges from textureless data and depth noise, this approach does not guarantee the reliability to extract local points. Unlike \cite{yang2012recognizing, xia2013spatio}, approaches in \cite{wang2012mining, oreifej2013hon4d} investigate to exploit depth information only. \cite{wang2012mining} designs a new feature called local occupancy pattern (LOP) to describe the local "depth appearance" for the joints. This feature can be suitable to characterize interactive information between the human subject and the environmental objects. \cite{oreifej2013hon4d} accumulates motion information and geometry cues using a histogram capturing the distribution of the surface normal orientation in the 4D space of depth, time, and spatial coordinates. The approaches show promising results on depth information. They, however, do not leverage the effectiveness of the intensity-based techniques. Therefore, we propose an effective method that can satisfy both mentioned factors.
\iffalse
\begin{figure}
	\centering
		\subfloat[Trajectory shape of action \textit{Forward Punch} on various views]{\label{subfig1:ForwardPunch_TrajShape}
		...
		\hfill
		\subfloat[Trajectory shape of action \textit{Hammer} on various views]{\label{subfig2:Hammer_TrajShape}
		...
		\hfill		
		\subfloat[From top view]{\label{subfig3:ForwardPunch_TrajShape}
...
After extracting trajectories, two kinds of descriptors: a trajectory shape descriptor and a trajectory-aligned descriptor can be adopted. In our experiments, we only use trajectory-aligned descriptors including the HOG \cite{dalal2005histograms}, the Histogram of Optical Flow (HOF) \cite{laptev2008learning}, and the Motion Boundary Histogram (MBH) \cite{dalal2006human}. The descriptors are computed within a space-time volume ($N \times N$ spatial pixels and $L$ temporal frames) around the trajectory. This volume is divided into a 3D grid (spatially $n_\sigma \times n_\sigma$ grid and temporally $n_\tau$ segments). The default settings of these parameters are $N$ = 32 pixels, $L$ = 15 frames, $n_\sigma$ = 2, and $n_\tau$ = 3.
...
Our proposed trajectory-based approach for human action recognition in depth data is as follow. At first, intensity representations are formed from the sequence of depth maps, as illustrated in figure \ref{lbl:Figure_ProposedMethod}.

At this step, to obtain an intensity representation from an view direction $\alpha$, corresponding to a view plane $\mathcal{P}(\alpha): ax + by + cx + d = 0$, in each depth map $t$, each point $P_t(x_t,y_t,z_t)$ is projected to $P_\mathcal{P}(x_\mathcal{P},y_\mathcal{P},z_\mathcal{P})$ on the view plane $\mathcal{P}(\alpha)$ by:
...
\begin{equation}
	v(P_\mathcal{P}) = \frac{(ax_t + by_t + cz_t + d)^2}{a^2 + b^2 + c^2}
\end{equation}
So, given a set of 3D points $\mathcal{S} = \{(x_t,y_t,z_t)\vert(x_t,y_t,z_t) \in t\}$, we have a projection $\mathcal{S}_\alpha = \{(x_\mathcal{P},y_\mathcal{P},z_\mathcal{P})\vert(x_\mathcal{P},y_\mathcal{P},z_\mathcal{P}) \in \mathcal{P}(\alpha)\}$. A set of the projections from a given sequence of depth maps is the expected intensity representation.

In particular, we choose three representations to represents for three view directions: front, side, and top in 3D space. With these view directions, the corresponding projections are, respectively:
\begin{equation}
\mathcal{S}_\text{front} = \{(x_t,y_t,0)\vert(x_t,y_t,0) \in \mathcal{P}:z=0\}
\end{equation}
\begin{equation}
\mathcal{S}_\text{side} = \{(0,y_t,z_t)\vert(0,y_t,z_t) \in \mathcal{P}:x=0\}
\end{equation}
\begin{equation}
\mathcal{S}_\text{top} = \{(x_t,0,z_t)\vert(x_t,0,z_t) \in \mathcal{P}:y=0\}
\end{equation}
...
\begin{equation}
	v(P_\text{top}) = y_t
\end{equation}
Forming the representations is necessary due to dimensional gap when we adapt 2D techniques for 3D data. After that, the dense trajectories \cite{wang2011densetraj} are extracted from the intensity representations. With this approach, we do not care the challenges for segmenting human body. Trajectory-aligned descriptors are computed then. At the next step, with each intensity representation, corresponding feature representation is quantized from raw trajectory features by applying a bag-of-words (BoW) model. An \textit{early fusion} scheme is used to generate the final feature representation for action in the sequence of depth maps (Fig. \ref{lbl:Figure_Framework}).
