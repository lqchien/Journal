...
%To obtain the 2D transformed videos, we build pseudo-views, which can capture the discriminative motions similar to observing actions under different directions.
...
\begin{figure*}[t]
	...
	\subfloat[Action \textit{Hammer}]{
		\includegraphics[width=0.4\textwidth]{Hammer3D.pdf}
		\label{subfig_Hammer3D}
	}
	\hspace{2cm}
	\subfloat[Action \textit{Forward Punch}]{
		\includegraphics[width=0.4\textwidth]{ForwardPunch3D.pdf}
	...
	\label{lbl:Figure_ProposedMethod}
\end{figure*}
...
In this section, we present a brief description about datasets used in our experiments.
Some example frames in the datasets are shown in figure \ref{Figure_ExampleFrames}.
In addition, we show how our framework works in step by step.
And parameters used in this framework are mentioned in detail.

\begin{figure}
	\centering
	\includegraphics[width=0.47\textwidth]{Figure_ExampleFrames.pdf}
	\caption{Example frames from different actions from MSR Action 3D dataset \cite{li2010action}, MSR Gesture 3D datset \cite{wang2012robust}, 3D Action Pairs dataset \cite{oreifej2013hon4d} and MSR Daily Activity 3D dataset \cite{wang2012mining}}
	\label{Figure_ExampleFrames}
\end{figure}

\subsection{Datasets}

\paragraph{\textbf{MSR Action 3D}}
This dataset \cite{li2010action} contains 20 actions, as showed in Table \ref{lbl:20actions}. Actions are performed by ten subjects for two or three times in the context of game console interaction. In total, there are 567 sequences of depth maps. The depth maps are shot at frame rate of 15 fps. The size of the depth map is $640 \times 480$, we resize into $320 \times 240$ to ensure processing efficiency.
...
\paragraph{\textbf{MSR Gesture 3D}}
The Gesture3D dataset \cite{wang2012robust} is a hand gesture dataset of depth sequences captured by a depth camera. This dataset contains a set of gesture defined by American Sign Language (ASL). In the dataset, there are 12 gestures as described in table \ref{tableGesture3D}. There are ten subjects, each performs each gesture two or three times. In total, the dataset contains 333 depth sequences. The main challenge in the dataset is self-occlusion issues. We follow the experimental settings in \cite{wang2012robust} to do evaluation on our approach.
...
\begin{table}[H]
	\begin{center}
		...
		\begin{tabular}{c|c|c|c}
		  ...
		  {\bf   ID  } & {\bf Gesture Name} &   {\bf   ID  } & {\bf Gesture Name} \\
		\hline
		             1 &      bathroom      &              7 &      past 	\\
	...
	\end{center}
	\caption{\label{tableGesture3D}12 ASL gestures in MSR Gesture 3D dataset}
\end{table}

\paragraph{\textbf{3D Action Pairs}}
...
%	\resizebox{\textwidth}{!}{
	...
	\end{center}
	\caption{\label{lbl:AS123ConfusionMatrix}Confusion matrices on three action subsets of MSR Action 3D dataset. Notice that action names are identified by indices of actions in table \ref{lbl:20actions}}
\end{table}
...
\begin{table}[h]
	\begin{center}
		...
		Bag of 3D Points \cite{li2010action} &         74.70  \\
		      ...
		      Space-Time Occupancy Patterns \cite{vieira2012stop} &         84.80  \\
		...
		Group Sparsity and Geometry Constraints \cite{luo2013group} &         96.70  \\
		\hline
		    {\it FRONT + SIDE} &   {\it 93.94 } \\
		    {\it \textbf{FRONT + TOP}} &   {\bf \textit{96.95} } \\
		    {\it FRONT + SIDE + TOP} &   {\it 96.67 } \\
		    		
		\end{tabular}%
		}
	\end{center}
	\caption{\label{lbl:Table_MBHvsSoAonFront}Comparison of accuracy on MSR Action 3D dataset. Notice that experimental results reported in this table is based on fusing feature representations from three views: front, side and top. Besides, we also use MBH descriptor only to calculate trajectory features.}
\end{table}
...
\begin{figure}[h]
	...
	\end{center}
	\caption{\label{lbl:Figure_MBHHOGHOF}Comparison of recognition accuracy on trajectory-aligned descriptors on MSR Action 3D dataset.}
\end{figure}
...
\begin{figure}[h]
	...
	\end{center}
	\caption{\label{lbl:Figure_CombinationsFRONTSIDETOP}Comparison of recognition accuracy on combinations of intensity representations on MSR Action 3D dataset.}
\end{figure}
